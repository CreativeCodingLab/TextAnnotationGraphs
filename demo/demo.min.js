  ...
  "eventData": {
   "comments": [
      ...
      [
         "E1",
         "FoundByRule",
         "Positive_activation_token_2_noun"
      ]
   ],
   "entities": [
      [
   ...
   "attributes": [],
   "text": "Induction of p21 by p53 following DNA damage inhibits both Cdk4 and Cdk2 activities",
   "relations": [],
   "triggers": [[
      "T6",
      "Positive_activation",
...
"use strict";var _classCallCheck2=_$interopRequireDefault_9(_$classCallCheck_7),_createClass2=_$interopRequireDefault_9(_$createClass_8),WordTag=function(){function WordTag(val,word){var _this=this,above=!(arguments.length>2&&void 0!==arguments[2])||arguments[2];if((0,_classCallCheck2.default)(this,WordTag),this.val=val,this.entity=word,this.position=above,word.svg){this.svg=word.svg.group().y(this.tagOffset),this.svgText=this.svg.text(this.val).addClass("tag-element").addClass(above?"word-tag":"word-tag syntax-tag"),this.ww=this.svgText.length();var svg=word.mainSVG;this.svgText.node.oncontextmenu=function(e){e.preventDefault(),svg.fire("tag-right-click",{object:_this,event:e})},this.svgText.click(function(e){return svg.fire("tag-edit",{object:_this})}),this.line=this.svg.path(),this.updateWordWidth()}else console.log("error: word must have an svg element")}return(0,_createClass2.default)(WordTag,[{key:"remove",value:function(){return this.entity.tag=null,this.entity.calculateBox(),this.svg.remove()}},{key:"updateWordWidth",value:function(){if(this.position){var ww=this.entity.ww;if(this.entity.val.length<9)this.line.plot("M0,25,l0,8");else{var diff=ww/2;this.line.plot("M0,25,c0,8,"+[diff,0,diff,8]+",M0,25,c0,8,"+[-diff,0,-diff,8])}}}},{key:"text",value:function(val){if(void 0===val)return this.svgText;if(this.val=val,this.svgText.text(this.val),this.ww=this.svgText.length(),this.editingRect){var bbox=this.svgText.bbox();bbox.width>0?this.editingRect.width(bbox.width+8).height(bbox.height+4).x(bbox.x-4).y(bbox.y-2):this.editingRect.width(10).x(-5)}}},{key:"changeEntity",value:function(word){this.entity&&(this.entity.tag=null),this.entity=word,this.entity.tag=this,this.entity.svg.add(this.svg)}},{key:"listenForEdit",value:function(){this.isEditing=!0;var bbox=this.svgText.bbox();this.svg.addClass("tag-element").addClass("editing"),this.editingRect=this.svg.rect(bbox.width+8,bbox.height+4).x(bbox.x-4).y(bbox.y-2).rx(2).ry(2).back()}},{key:"stopEditing",value:function(){this.isEditing=!1,this.svg.removeClass("editing"),this.editingRect.remove(),this.editingRect=null,this.val=this.val.trim(),this.val?this.entity.calculateBox():this.remove()}},{key:"tagOffset",get:function(){return this.position?-28:20}}]),WordTag}();var _$WordTag_85=WordTag;

"use strict";var ___classCallCheck2_86=_$interopRequireDefault_9(_$classCallCheck_7),___createClass2_86=_$interopRequireDefault_9(_$createClass_8),_tag=_$interopRequireDefault_9(_$WordTag_85),Word=(_$interopRequireWildcard_10(_$svg_81),_$interopRequireWildcard_10(_$svgDraggable_80),function(){function Word(val,idx,tag,svg,row){(0,___classCallCheck2_86.default)(this,Word),this.eventIds=[],this.val=val,this.idx=idx,this.x=0,this.slot=0,this.boxWidth=0,this.boxHeight=0,this.descendHeight=0,this.isPunct=1===val.length&&val.charCodeAt(0)<65,this.clusters=[],this.links=[],tag&&this.setTag(tag),svg&&this.init(svg,row)}return(0,___createClass2_86.default)(Word,[{key:"addEventId",value:function(id){this.eventIds.indexOf(id)<0&&this.eventIds.push(id)}},{key:"setSyntaxId",value:function(id){this.syntaxId=id}},{key:"setTag",value:function(tag){return this.svg?(tag instanceof _tag.default?this.tag=tag:this.tag instanceof _tag.default?this.tag.text(tag):this.tag=new _tag.default(tag,this),this.calculateBox()):this.tag=tag,this.tag}},{key:"setSyntaxTag",value:function(tag){return this.svg?(tag instanceof _tag.default?this.syntaxTag=tag:this.syntaxTag instanceof _tag.default?this.syntaxTag.text(tag):this.tag=new _tag.default(tag,this,!1),this.calculateBox()):this.syntaxTag=tag,this.syntaxTag}},{key:"init",value:function(svg){var _this=this;this.mainSVG=svg,this.svg=svg.group().addClass("tag-element").addClass("word"),this.svgText=this.svg.text(this.val).addClass("tag-element").addClass("word-text"),!this.tag||this.tag instanceof _tag.default||(this.tag=new _tag.default(this.tag,this)),!this.syntaxTag||this.syntaxTag instanceof _tag.default||(this.syntaxTag=new _tag.default(this.syntaxTag,this,!1)),this.clusters.forEach(function(cluster){cluster.init(_this)}),this.calculateBox(),this.svg.y(-this.svgText.bbox().y2);var x=0,mousemove=!1;this.svgText.draggable().on("dragstart",function(e){mousemove=!1,x=e.detail.p.x,svg.fire("word-move-start")}).on("dragmove",function(e){e.preventDefault();var dx=e.detail.p.x-x;x=e.detail.p.x,svg.fire("word-move",{object:_this,x:dx}),0!==dx&&(mousemove=!0)}).on("dragend",function(e){svg.fire("word-move-end",{object:_this,clicked:!1===mousemove})}),this.svgText.dblclick(function(e){return svg.fire("build-tree",{object:_this,event:e})}),this.svgText.node.oncontextmenu=function(e){e.preventDefault(),svg.fire("word-right-click",{object:_this,event:e})}}},{key:"redrawLinks",value:function(){var _this2=this;this.links.forEach(function(l){return l.draw(_this2)}),this.redrawClusters()}},{key:"redrawClusters",value:function(){var _this3=this;this.clusters.forEach(function(cluster){cluster.endpoints.indexOf(_this3)>-1&&cluster.draw()})}},{key:"move",value:function(x){this.x=x,this.svg.transform({x:this.boxWidth/2+this.x}),this.redrawLinks()}},{key:"dx",value:function(x){this.move(this.x+x)}},{key:"calculateBox",value:function(){var minWidth=this.tag instanceof _tag.default?Math.max(this.tag.ww,this.ww):this.ww,diff=this.boxWidth-minWidth;this.boxWidth-=diff,this.descendHeight=this.syntaxTag instanceof _tag.default?this.syntaxTag.svgText.bbox().height:0,this.boxHeight=this.svg.bbox().height-this.descendHeight,this.dx(diff/2),this.mainSVG.fire("word-move",{object:this,x:0})}},{key:"moveToRow",value:function(row,i){var ignorePosition=!(arguments.length>2&&void 0!==arguments[2])||arguments[2];this.row.removeWord(this),row.addWord(this,i,ignorePosition)}},{key:"absoluteDescent",get:function(){return this.row?this.row.ry+this.row.rh+this.descendHeight+8:0}},{key:"absoluteY",get:function(){return this.row?this.row.ry+this.row.rh-this.boxHeight:0}},{key:"cx",get:function(){return this.x+this.boxWidth/2}},{key:"ww",get:function(){return this.svgText.length()}}]),Word}());var _$Word_86=Word;

"use strict";var ___classCallCheck2_83=_$interopRequireDefault_9(_$classCallCheck_7),___createClass2_83=_$interopRequireDefault_9(_$createClass_8),___tag_83=_$interopRequireDefault_9(_$WordTag_85),_word=_$interopRequireDefault_9(_$Word_86),Link=(_$interopRequireWildcard_10(_$svg_81),_$interopRequireWildcard_10(_$svgDraggable_80),function(){function Link(eventId,trigger,args,reltype){var _this=this,top=!(arguments.length>4&&void 0!==arguments[4])||arguments[4];(0,___classCallCheck2_83.default)(this,Link),this.eventId=eventId,this.trigger=trigger,this.arguments=args.sort(function(a,b){return a.anchor.idx-b.anchor.idx}),this.links=[],this.reltype=reltype,this.top=top,this.visible=!0,this.resetSlotRecalculation(),this.top?(this.trigger&&this.trigger.links.push(this),this.arguments.forEach(function(arg){arg.anchor.links.push(_this)})):(this.trigger.links.push(this),this.arguments.forEach(function(arg){return arg.anchor.links.push(_this)})),this.endpoints=this.getEndpoints(),this.mainSVG=null,this.svg=null,this.handles=[],this.line=null,this.svgTexts=[]}return(0,___createClass2_83.default)(Link,[{key:"init",value:function(svg){var _this2=this;if(this.arguments.sort(function(a,b){return a.anchor.idx-b.anchor.idx}),this.mainSVG=svg,this.svg=svg.group().addClass("tag-element").addClass(this.top?"link":"link syntax-link"),this.visible||this.svg.hide(),this.trigger){var _x=this.trigger.cx,y=this.top?this.trigger.absoluteY:this.trigger.absoluteDescent;this.handles.push({anchor:this.trigger,x:_x,y:y,offset:null})}if(this.arguments.forEach(function(arg){var x=arg.anchor.cx,y=_this2.top?arg.anchor.absoluteY:arg.anchor.absoluteDescent;if(_this2.handles.push({anchor:arg.anchor,x:x,y:y,offset:null}),_this2.trigger){var text=_this2.svg.text(arg.type).y(-7).addClass("tag-element").addClass("link-text");_this2.svgTexts.push(text)}}),this.reltype){var text=this.svg.text(this.reltype).y(-7).addClass("tag-element").addClass("link-text");this.svgTexts.push(text)}this.svgTexts.forEach(function(text){text.node.oncontextmenu=function(e){_this2.selectedLabel=text,e.preventDefault(),svg.fire("link-label-right-click",{object:_this2,type:"text",event:e})},text.click(function(e){return svg.fire("link-label-edit",{object:_this2,text:text,event:e})}),text.dblclick(function(e){return svg.fire("build-tree",{object:_this2,event:e})})}),this.line=this.svg.path().addClass("tag-element").addClass("polyline");var draggedHandle=null,x=0;this.line.dblclick(function(e){return svg.fire("build-tree",{object:_this2,event:e})}),this.line.node.oncontextmenu=function(e){e.preventDefault(),svg.fire("link-right-click",{object:_this2,type:"link",event:e})},this.line.draggable().on("dragstart",function(e){var closestHandle=_this2.handles.reduce(function(acc,val){return Math.abs(val.x-e.detail.p.x)<Math.abs(acc.x-e.detail.p.x)?val:acc},_this2.handles[0]);Math.abs(closestHandle.x-e.detail.p.x)<5&&(draggedHandle=closestHandle,x=e.detail.p.x)}).on("dragmove",function(e){if(e.preventDefault(),draggedHandle){var dx=e.detail.p.x-x;x=e.detail.p.x,draggedHandle.offset+=dx;var anchor=draggedHandle.anchor;if(anchor instanceof Link){var _handles=anchor.handles.filter(function(h){return h.anchor.row.idx===anchor.handles[0].anchor.row.idx}).sort(function(a,b){return a.x-b.x}),_min=_handles[0].x,_max=_handles[_handles.length-1].x;_handles.length<anchor.handles.length&&(_max=_this2.mainSVG.width());var _cx=draggedHandle.anchor.cx;draggedHandle.offset=Math.min(_max-_cx,Math.max(_min-_cx,draggedHandle.offset))}else{var halfWidth=anchor.boxWidth/2;_this2.top&&anchor.tag instanceof ___tag_83.default?halfWidth=anchor.tag.ww/2:!_this2.top&&anchor.syntaxTag instanceof ___tag_83.default&&(halfWidth=anchor.syntaxTag.ww/2),halfWidth=Math.max(halfWidth,13),draggedHandle.offset=draggedHandle.offset<0?Math.max(3-halfWidth,draggedHandle.offset):Math.min(halfWidth-3,draggedHandle.offset)}var handles=_this2.handles.filter(function(h){return h.anchor.row.idx===_this2.handles[0].anchor.row.idx}).sort(function(a,b){return a.x-b.x}),min=handles[0].x,max=handles[handles.length-1].x;handles.length<_this2.handles.length&&(max=_this2.mainSVG.width());var cx=_this2.cx;_this2.links.forEach(function(link){link.handles.forEach(function(h){h.anchor===_this2&&(h.offset=Math.min(max-cx,Math.max(min-cx,h.offset)))})}),_this2.draw(draggedHandle.anchor)}}).on("dragend",function(){draggedHandle=null})}},{key:"toggle",value:function(){this.visible=!this.visible,this.visible?this.show():this.hide()}},{key:"show",value:function(){this.visible=!0,this.svg&&(this.svg.show(),this.draw())}},{key:"hide",value:function(){this.visible=!1,this.svg&&this.svg.hide()}},{key:"draw",value:function(anchor){var _this3=this;if("R2"===this.eventId&&(window.debugLink=this,window.debugAnchor=anchor),anchor){var h=this.handles.find(function findAnchor(thisHandle){return thisHandle.anchor===anchor?thisHandle:thisHandle.anchor instanceof Link&&thisHandle.anchor.handles.find(findAnchor)?thisHandle:void 0});console.log(this.eventId,h),h&&(h.x=anchor.cx+h.offset,h.y=this.top?anchor.absoluteY:anchor.absoluteDescent)}else this.handles.forEach(function(h){if(null===h.offset){var l=h.anchor.links.sort(function(a,b){return a.slot-b.slot}).filter(function(link){return link.top==_this3.top});l.length>1?h.anchor instanceof Link&&h.anchor.trigger.idx===h.anchor.endpoints[0].idx?h.offset=l.indexOf(_this3)/l.length*2*10:h.anchor instanceof Link&&h.anchor.trigger.idx===h.anchor.endpoints[1].idx?h.offset=l.indexOf(_this3)/l.length*2*-10:(l=l.filter(function(link){return h.anchor.idx>link.endpoints[0].idx==h.anchor.idx>_this3.endpoints[0].idx}),h.offset=(l.indexOf(_this3)+.5)/l.length*(h.anchor.idx>_this3.endpoints[0].idx?-10:10)):h.offset=0}});if(this.visible){if(this.line){var width=this.mainSVG.width(),d="";if(this.trigger)for(var y=this.getY(this.handles[1]),rowCrossed=!1,i=0,il=this.arguments.length;i<il;++i){var leftOfTrigger=this.arguments[i].anchor.idx<this.trigger.idx,dx=leftOfTrigger?5:-5,textlen=leftOfTrigger?this.svgTexts[i].length():-this.svgTexts[i].length(),handle1=this.handles[i+1];i>0?(rowCrossed&&(rowCrossed=!1,d+="L"+[width,y]+"M0,",d+=y=this.getY(handle1)),d+=leftOfTrigger?"L"+[handle1.x+dx,y]:"L"+[handle1.x+dx+textlen,y]):leftOfTrigger||(y=this.getY(this.handles[0]),d+="M"+[this.handles[0].x,this.handles[0].y]+"C"+[this.handles[0].x,y,this.handles[0].x,y,this.handles[0].x-dx,y],this.handles[0].anchor.row.idx<this.handles[1].anchor.row.idx&&(d+="L"+[width,y]+"M0,",d+=y=this.getY(this.handles[1])),d+="L"+[this.handles[1].x+dx+textlen,y]),this.svgTexts[i].x(handle1.x+dx+textlen/2).y(y-10);d+=this.arrowhead(handle1);var handlePrecedesTrigger=leftOfTrigger&&(i+2>il||this.arguments[i+1].anchor.idx>=this.trigger.idx);if(rowCrossed=handlePrecedesTrigger&&this.handles[0].anchor.row.idx!=handle1.anchor.row.idx||!handlePrecedesTrigger&&i+1<il&&this.handles[i+2].anchor.row.idx!=handle1.anchor.row.idx,handlePrecedesTrigger&&rowCrossed){var tempY=this.getY(handle1);y=this.getY(this.handles[0]),d+="M"+[handle1.x,handle1.y]+"C"+[handle1.x,tempY,handle1.x,tempY,handle1.x+dx,tempY]+"m"+[textlen,0]+"L"+[width,tempY]+"M"+[0,y],rowCrossed=!1,this.svgTexts[i].y(tempY-10)}else d+="M"+[handle1.x,handle1.y]+"C"+[handle1.x,y,handle1.x,y,handle1.x+dx,y],leftOfTrigger&&(d+="m"+[textlen,0]);handlePrecedesTrigger&&(i+1<il?(d+="L"+[this.handles[0].x-dx,y]+"c"+[dx,0,dx,0,dx,this.handles[0].y-y]+"m"+[dx,0]+"l"+[-2*dx,0]+"m"+[dx,0]+"C"+[this.handles[0].x,y,this.handles[0].x,y,this.handles[0].x+dx,y],rowCrossed=this.handles[i+2].anchor.row.idx!=this.handles[0].anchor.row.idx):d+="L"+[this.handles[0].x-dx,y]+"c"+[dx,0,dx,0,dx,this.handles[0].y-y])}else if(this.reltype){var avg,_y=this.getY(this.handles[0]),endHandle=this.handles[this.handles.length-1],_textlen=this.svgTexts[0].length();if(this.handles[0].anchor.row.idx===endHandle.anchor.row.idx){var textLeft=(avg=this.handles.reduce(function(acc,h){return acc+h.x},0)/this.arguments.length)-_textlen/2;d="M"+[this.handles[0].x,this.handles[0].y]+(textLeft<this.handles[0].x?"L"+[this.handles[0].x,_y]+"M"+[endHandle.x,_y]+"L"+[endHandle.x,endHandle.y]:"C"+[this.handles[0].x,_y,this.handles[0].x,_y,Math.min(textLeft,this.handles[0].x+5),_y]+"L"+[textLeft,_y]+"m"+[_textlen,0]+"L"+[Math.max(textLeft+_textlen,endHandle.x-5),_y]+"C"+[endHandle.x,_y,endHandle.x,_y,endHandle.x,endHandle.y])+this.arrowhead(this.handles[0])+this.arrowhead(endHandle)}else{avg=(this.handles[0].x+width)/2,d="M"+[this.handles[0].x,this.handles[0].y]+"C"+[this.handles[0].x,_y,this.handles[0].x,_y,this.handles[0].x+5,_y]+"L"+[avg-_textlen/2,_y]+"m"+[_textlen,0]+"L"+[width,_y];var _tempY=this.getY(endHandle);d+="M0,"+_tempY+"L"+[endHandle.x-5,_tempY]+"C"+[endHandle.x,_tempY,endHandle.x,_tempY,endHandle.x,endHandle.y]+this.arrowhead(this.handles[0])+this.arrowhead(endHandle)}this.svgTexts[0].x(avg).y(_y-10)}window.debugPath=d,this.line.plot(d)}this.links.forEach(function(l){return l.draw(_this3)})}}},{key:"getY",value:function(handle){var r=handle.anchor.row;return this.top?r.rh+r.ry-45-15*this.slot:r.rh+r.ry+25-15*this.slot}},{key:"arrowhead",value:function(handle){return this.top?"M"+[handle.x-4,handle.y-4]+"l"+[4,6]+"l"+[4,-6]:"M"+[handle.x-4,handle.y+4]+"l"+[4,-6]+"l"+[4,6]}},{key:"remove",value:function(){this.svg.remove();var self=this;function detachLink(anchor){var i=anchor.links.indexOf(self);i>-1&&anchor.links.splice(i,1)}this.trigger&&detachLink(this.trigger),this.arguments.forEach(function(arg){return detachLink(arg.anchor)})}},{key:"resetSlotRecalculation",value:function(){var _this4=this;this.isRecalculated=!1,this.slot=0,this.top?(this.trigger&&(this.slot=this.trigger.slot),this.arguments.forEach(function(arg){arg.anchor.slot>_this4.slot&&(_this4.slot=arg.anchor.slot)}),this.slot+=1):this.slot=-1}},{key:"recalculateSlots",value:function(words){var ep=this.endpoints;if(this.isRecalculated)return[{slot:this.slot,endpoints:ep}];this.isRecalculated=!0;var wordArray=words.slice(ep[0].idx,ep[1].idx+1),self=this,slots=[];return wordArray.forEach(function(word){word.links.forEach(function(l){l===self||l.top!==self.top||l.endpoints[0].idx>=ep[1].idx||l.endpoints[1].idx<=ep[0].idx||[].push.apply(slots,l.recalculateSlots(words))})}),function incrementSlot(l){for(;slots.find(function(s){return s.slot===l.slot&&!(s.endpoints[0].idx>=l.endpoints[1].idx||s.endpoints[1].idx<=l.endpoints[0].idx)});)l.slot+=l.top?1:-1;slots.push({slot:l.slot,endpoints:l.endpoints}),l.links.forEach(function(link){link.top===l.top&&incrementSlot(link)})}(this),slots}},{key:"listenForEdit",value:function(e){this.isEditing=!0;var bbox=e.detail.text.bbox();e.detail.text.addClass("tag-element").addClass("editing-text"),this.editingText=e.detail.text,this.editingRect=this.svg.rect(bbox.width+8,bbox.height+4).x(bbox.x-4).y(bbox.y-2).rx(2).ry(2).addClass("tag-element").addClass("editing-rect").back()}},{key:"text",value:function(str){if(this.editingText){if(void 0===str)return this.editingText;this.editingText.text(str)}}},{key:"stopEditing",value:function(){this.isEditing=!1,this.editingText.removeClass("editing-text"),this.editingRect.remove(),this.editingRect=this.editingText=null,this.draw()}},{key:"getEndpoints",value:function(){var minWord=null,maxWord=null;return this.trigger&&(minWord=maxWord=this.trigger),this.arguments.forEach(function(arg){if(arg.anchor instanceof Link){var endpts=arg.anchor.getEndpoints();(!minWord||minWord.idx>endpts[0].idx)&&(minWord=endpts[0]),(!maxWord||maxWord.idx<endpts[1].idx)&&(maxWord=endpts[1])}else(!minWord||minWord.idx>arg.anchor.idx)&&(minWord=arg.anchor),(!maxWord||maxWord.idx<arg.anchor.idx)&&(maxWord=arg.anchor)}),[minWord,maxWord]}},{key:"rootWord",get:function(){return this.trigger?this.trigger:this.arguments[0].anchor instanceof _word.default?this.arguments[0].anchor:this.arguments[0].anchor.rootWord}},{key:"idx",get:function(){return this.rootWord.idx}},{key:"row",get:function(){return this.rootWord.row}},{key:"cx",get:function(){return this.line&&this.trigger?this.trigger.cx+this.handles[0].offset:this.rootWord.cx}},{key:"absoluteY",get:function(){return this.rootWord.row.rh+this.rootWord.row.ry-45-15*this.slot}},{key:"val",get:function(){return this.reltype||this.trigger.reltype||this.trigger.tag&&this.trigger.tag.val||this.trigger.val}}]),Link}());var _$Link_83=Link;

"use strict";var ___classCallCheck2_89=_$interopRequireDefault_9(_$classCallCheck_7),_link=_$interopRequireDefault_9(_$Link_83);var _$labelmanager_89=function(){var _svg,activeObject=null,string=null,originalString=null,MAX_WIDTH=22,Key={delete:8,tab:9,enter:13,escape:27,left:37,up:38,right:39,down:40},KeyValues=Object.values(Key);function listenForEdit(e){(activeObject=e.detail.object).listenForEdit(e),originalString=e.detail.object.val,string=null}function stopEditing(){if(activeObject&&activeObject.isEditing){var text=activeObject.text();!text||activeObject instanceof _link.default||_svg.fire("label-updated",{object:text,label:text.text()}),activeObject.stopEditing(),activeObject=null,originalString=string=null}}function updateString(s){string=s.slice(0,MAX_WIDTH),activeObject.text(string)}return document.addEventListener("keydown",function(e){if(activeObject&&activeObject.isEditing&&KeyValues.indexOf(e.keyCode)>-1)switch(e.keyCode){case Key.delete:null===string&&(string=originalString),updateString(string.slice(0,-1));break;case Key.tab:break;case Key.enter:stopEditing();break;case Key.escape:updateString(originalString),stopEditing();break;case Key.right:null===string&&updateString(originalString)}}),document.addEventListener("keypress",function(e){activeObject&&activeObject.isEditing&&(32===e.which&&e.preventDefault(),e.which>31&&!e.ctrlKey&&!e.metaKey&&!e.altKey&&(null===string&&(string=""),updateString(string+String.fromCharCode(e.which))))}),document.addEventListener("mousedown",stopEditing),function LabelManager(svg){(0,___classCallCheck2_89.default)(this,LabelManager),_svg=svg,svg.on("tag-edit",listenForEdit),svg.on("link-label-edit",listenForEdit),this.stopEditing=stopEditing}}();
...
"use strict";var ___classCallCheck2_84=_$interopRequireDefault_9(_$classCallCheck_7),___createClass2_84=_$interopRequireDefault_9(_$createClass_8),Row=(_$interopRequireWildcard_10(_$svg_81),_$interopRequireWildcard_10(_$svgDraggable_80),function(){function Row(svg){var idx=arguments.length>1&&void 0!==arguments[1]?arguments[1]:0,ry=arguments.length>2&&void 0!==arguments[2]?arguments[2]:0,rh=arguments.length>3&&void 0!==arguments[3]?arguments[3]:100;(0,___classCallCheck2_84.default)(this,Row),this.idx=idx,this.ry=ry,this.rh=rh,this.rw=0,this.words=[],this.maxSlot=0,this.minSlot=0,this.svg=null,this.draggable=null,this.wordGroup=null,svg&&this.init(svg)}return(0,___createClass2_84.default)(Row,[{key:"init",value:function(svg){var _this=this;this.svg=svg.group().transform({y:this.ry}).addClass("tag-element").addClass("row"),this.wordGroup=this.svg.group().y(this.rh),this.rw=svg.width(),this.draggable=this.svg.line(0,0,this.rw,0).y(this.rh).addClass("tag-element").addClass("row-drag").draggable();var y=0;this.draggable.on("dragstart",function(e){y=e.detail.p.y}).on("dragmove",function(e){e.preventDefault();var dy=e.detail.p.y-y;y=e.detail.p.y,svg.fire("row-resize",{object:_this,y:dy})})}},{key:"remove",value:function(){return this.svg.remove()}},{key:"dy",value:function(y){this.ry+=y,this.svg.transform({y:this.ry})}},{key:"move",value:function(y){this.ry=y,this.svg.transform({y:this.ry})}},{key:"height",value:function(rh){this.rh=rh,this.wordGroup.y(this.rh),this.draggable.y(this.rh)}},{key:"width",value:function(rw){this.rw=rw,this.draggable.attr("x2",this.rw)}},{key:"addWord",value:function(word,i,ignorePosition){if(isNaN(i)&&(i=this.words.length),word.row=this,this.words.splice(i,0,word),this.wordGroup.add(word.svg),!ignorePosition)return this.moveWordRight(word)}},{key:"moveWordRight",value:function(word,x){var i=this.words.indexOf(word),prevWord=this.words[i-1];if(x){if((x=Math.min(this.rw,x))+word.boxWidth>this.rw-10)return i;word.move(x),++i,prevWord=word}else prevWord||(word.move(10),++i,prevWord=word);for(;i<this.words.length;){var _word=this.words[i],dx=prevWord.x+prevWord.boxWidth+(_word.isPunct?0:5);if(_word.x>dx)return;if(dx+_word.boxWidth>this.rw-10)return i;_word.move(dx),prevWord=this.words[i],++i}}},{key:"removeWord",value:function(word){return this.words.splice(this.words.indexOf(word),1),this.wordGroup.removeElement(word.svg),word}},{key:"removeLastWord",value:function(){var word=this.words.pop();return this.wordGroup.removeElement(word.svg),word}},{key:"removeFirstWord",value:function(){var word=this.words.shift();return this.wordGroup.removeElement(word.svg),word}},{key:"ry2",get:function(){return this.ry+this.rh+20-15*this.minSlot}},{key:"minHeight",get:function(){return 60+15*this.maxSlot}}]),Row}());var _$Row_84=Row;

"use strict";var _slicedToArray2=_$interopRequireDefault_9(_$slicedToArray_13),___classCallCheck2_90=_$interopRequireDefault_9(_$classCallCheck_7),___createClass2_90=_$interopRequireDefault_9(_$createClass_8),_row=_$interopRequireDefault_9(_$Row_84),___link_90=_$interopRequireDefault_9(_$Link_83),RowManager=function(){function RowManager(svg){(0,___classCallCheck2_90.default)(this,RowManager),this._svg=svg,this._rows=[]}return(0,___createClass2_90.default)(RowManager,[{key:"resizeAll",value:function(){var _this=this;this.width(this._svg.width()),this._rows.forEach(function(row){_this.recalculateRowSlots(row)}),this.resizeRow(0)}},{key:"resizeRow",value:function(i){var dy=arguments.length>1&&void 0!==arguments[1]?arguments[1]:0,row=this._rows[i];if(void 0!==row){if(i>0){var adjust=this._rows[i-1].ry2+10-row.ry;row.move(row.ry+adjust),row.height(row.rh-adjust)}dy=Math.max(-row.rh+row.minHeight,dy),row.height(row.rh+dy),row.words.forEach(function(word){return word.redrawLinks()});for(var j=i+1;j<this._rows.length;++j)this._rows[j-1].ry2+10>this._rows[j].ry+dy?this._rows[j].move(this._rows[j-1].ry2+10):this._rows[j].dy(dy),this._rows[j].words.forEach(function(word){return word.redrawLinks()});this._svg.height(this.lastRow.ry2+10+20)}}},{key:"width",value:function(rw){var _this2=this;this._rows.forEach(function(row){row.width(rw);var i=row.words.findIndex(function(w){return w.x>rw});i>0?_this2.moveWordOnRow(row.words[i-1],0):row.words.forEach(function(word){word.links.forEach(function(l){l.endpoints[1].row!==l.endpoints[0].row&&l.draw(this)}),word.redrawClusters()})})}},{key:"appendRow",value:function(){var lr=this.lastRow,row=lr?new _row.default(this._svg,lr.idx+1,lr.ry2+10):new _row.default(this._svg);return this._rows.push(row),this._svg.height(row.ry2+10+20),row}},{key:"removeRow",value:function(){this._rows.pop().remove(),this.lastRow&&this._svg.height(this.lastRow.ry2+10+20)}},{key:"addWordToRow",value:function(word,row,i,ignorePosition){isNaN(i)&&(i=row.words.length);var slots=this.getSlotRange([0,0],word);!word.row||word.row===row||slots[0]!==word.row.minSlot&&word.row.maxSlot!==slots[1]||this.recalculateRowSlots(word.row),(row.minSlot>slots[0]||row.maxSlot<slots[1])&&(row.minSlot>slots[0]&&(row.minSlot=slots[0]),row.maxSlot<slots[1]&&(row.maxSlot=slots[1]),this.resizeRow(row.idx));for(var overflow=row.addWord(word,i,ignorePosition);overflow<row.words.length;)this.moveWordDownARow(row.idx)}},{key:"moveWordOnRow",value:function(word,dx){var row=word.row;row&&(dx>=0?this.moveWordRight(row,dx,word):dx<0&&this.moveWordLeft(row,-dx,row.words.indexOf(word)))}},{key:"moveWordRight",value:function(row,dx,word){for(var overflow=row.moveWordRight(word,dx+word.x);overflow<row.words.length;)this.moveWordDownARow(row.idx)}},{key:"moveWordLeft",value:function(row,dx,i,overflow){if(!row)return!1;var x,finalRow,fitsOnRow=!0,j=i,words=overflow?row.words.concat(overflow):row.words;if(overflow||!words[i]){x=row.rw-10,j=i=words.length-1;var lastWord=row.words[row.words.length-1];dx=(lastWord?lastWord.x+lastWord.boxWidth:0)-x}else x=words[i].x+words[i].boxWidth-dx;for(;j>=0;){words[j];if(x-=words[j].boxWidth,j<row.words.length&&x>=words[j].x){finalRow=!0;break}if(x<10){fitsOnRow=this.moveWordLeft(this._rows[row.idx-1],null,null,words.slice(0,j+1));break}!1===words[j].isPunct&&(x-=5),--j}if(!fitsOnRow)return!1;if(overflow)for(x=row.rw-10;i>j;)x-=words[i].boxWidth,words[i].move(x),words[i].isPunct||(x-=5),--i;else for(;i>j;)words[i]&&words[i].dx(-dx),--i;if(!finalRow){for(;j>=0;)this.moveWordUpARow(row.idx),--j;0===row.words.length&&row.idx===this._rows.length-1&&this.removeRow()}return!0}},{key:"moveWordUpARow",value:function(index){if(this._rows[index-1]){var removedWord=this._rows[index].removeFirstWord();this.addWordToRow(removedWord,this._rows[index-1],void 0,!0),removedWord.redrawClusters(),removedWord.redrawLinks()}}},{key:"moveWordDownARow",value:function(index){var nextRow=this._rows[index+1]||this.appendRow();this.addWordToRow(this._rows[index].removeLastWord(),nextRow,0)}},{key:"getSlotRange",value:function(acc,anchor){var _this3=this;if(anchor instanceof ___link_90.default&&!anchor.visible)return[acc[0],acc[1]];if(0===anchor.links.length)return[Math.min(acc[0],anchor.slot),Math.max(acc[1],anchor.slot)];var a=anchor.links.reduce(function(acc,val){return _this3.getSlotRange(acc,val)},[0,0]);return[Math.min(acc[0],a[0]),Math.max(acc[1],a[1])]}},{key:"recalculateRowSlots",value:function(row){var _this4=this,_row$words$reduce=row.words.reduce(function(acc,val){return _this4.getSlotRange(acc,val)},[0,0]),_row$words$reduce2=(0,_slicedToArray2.default)(_row$words$reduce,2);row.minSlot=_row$words$reduce2[0],row.maxSlot=_row$words$reduce2[1]}},{key:"lastRow",get:function(){return this._rows[this._rows.length-1]}},{key:"rows",get:function(){return this._rows}}]),RowManager}();var _$RowManager_90=RowManager;
